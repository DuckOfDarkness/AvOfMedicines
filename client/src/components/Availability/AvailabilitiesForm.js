...
export default function MedicinesForm() {
    ...
    const currentFormMode = avId ? formMode.EDIT : formMode.NEW
    ...
    const [idWholesaler, setIdWholesaler] = useState('')
    const [allWholesalers, setAllWholesalers] = useState({})
    const [wholesaler, setWholesaler] = useState({
        name: '',
        nip:''
    })
    const [selectedWholesalerOption, setSelectedWholesalerOption] = useState(0)
    ...
    const [selectedMedicineOption, setSelectedMedicineOption] = useState(0)
    ...
    let medicineError, wholesalerError, amountError, dateOfPurchaseError;

    let errors = [medicineError, wholesalerError, amountError, dateOfPurchaseError]

    useEffect(() => {
        if (currentFormMode === formMode.EDIT && !isLoaded) {
            getAvailabilitiesByIdApiCall(avId)
                .then((res) => (res.json()))
                .then((data) => {
                    setAvailabilities(data)
                    setSelectedMedicineOption(data)
                    setSelectedWholesalerOption()
                })
        } else {
            setMedicines({
                name: '',
