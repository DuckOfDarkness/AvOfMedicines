...
function checkIfTheNumberIsNegative(number){
    return number < 0;
}
 ...
 */
function checkIfItContainsOnly(onlyWhat, value) {
    if (!value) {
        return false;
    }
    if (onlyWhat === "numbers") {
...
function checkThatItStartsWithACapitalLetter(value) {
    if (!value) {
        return false;
    }
    value = value.toString().trim();
 ...
 */
function checkThatDateIs_ThanTheCurrent(inPlaceOf_ ,value) {
    if (!value) {
        return false;
    }
    const pattern = /(\d{4})-(\d{2})-(\d{2})/;
